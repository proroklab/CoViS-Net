...
class EquivariantBlock(Block):
    def forward(self, xi: Tensor, xj: Tensor) -> Tensor:
        def attn_residual_func(xi, xj: Tensor) -> Tensor:
            xin = self.norm1(xi)
            xjn = self.norm1(xj)
            return self.ls1(self.attn(xin, xjn))

        def ffn_residual_func(x: Tensor) -> Tensor:
            return self.ls2(self.mlp(self.norm2(x)))

        xi = xi + attn_residual_func(xi, xj)
        xi = xi + ffn_residual_func(xi)
        return xi
